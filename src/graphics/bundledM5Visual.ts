...
interface CompactMaterialRecord {
  name: string;
  shader: string;
  blendMode: number;
}
...
class BinaryReader {
  ...
  string(): string {
    return TEXT_DECODER.decode(this.take(this.u8()));
  }
...
function materialFor(record: CompactMaterialRecord): THREE.MeshStandardMaterial {
  ...
  if (name === 'carpaint') {
    ...
    material.color.set(0x161a20);
    material.metalness = 0.55;
    material.roughness = 0.22;
  }

  if (record.blendMode !== 0 && !material.transparent) {
    material.transparent = true;
...
function parseCompactM5(bytes: Uint8Array): Kn5VisualResult {
  ...
  if (version !== 2) throw new Error(`Unsupported bundled BMW version: ${version}`);

  const materialCount = reader.u16();
  const meshCount = reader.u16();
  const materialRecords: CompactMaterialRecord[] = [];

  for (let i = 0; i < materialCount; i += 1) {
    materialRecords.push({
      name: reader.string(),
      shader: reader.string(),
      blendMode: reader.i16(),
    });
  }
  ...
  for (let meshIndex = 0; meshIndex < meshCount; meshIndex += 1) {
    ...
    const minX = reader.f32();
    const minY = reader.f32();
    const minZ = reader.f32();
    const maxX = reader.f32();
    const maxY = reader.f32();
    ...
    const mesh = new THREE.Mesh(geometry, materials[materialId] ?? fallback);
    mesh.name = name;
  ...
  if (reader.remaining() !== 0) {
    throw new Error(`Bundled BMW visual has ${reader.remaining()} unexpected trailing bytes.`);
  }
...
export async function loadBundledM5Visual(_config: VehicleConfig): Promise<Kn5VisualResult> {
  return parseCompactM5(await loadBundledBytes());
}
