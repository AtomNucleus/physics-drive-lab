...
function materialFor(record: CompactMaterialRecord): THREE.MeshStandardMaterial {
  ...
  if (record.blendMode !== 0 && !material.transparent) { material.transparent = true; material.opacity = 0.88; material.depthWrite = false; }
  return material;
}
...
function parseCompactM5(bytes: Uint8Array): Kn5VisualResult {
  ...
  for (let meshIndex = 0; meshIndex < meshCount; meshIndex += 1) {
    ...
    const minX = reader.f32(), minY = reader.f32(), minZ = reader.f32();
    const maxX = reader.f32(), maxY = reader.f32(), maxZ = reader.f32();
