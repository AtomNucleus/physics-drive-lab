...
export async function writeTrackPackage(model: Kn5Model, surfacesIni: AcSurfaceDefinition[], options: WriterOptions) {
  await mkdir(options.outputDir, { recursive: true });
  await mkdir(join(options.outputDir, 'textures'), { recursive: true });
  for (const texture of model.textures) await writeFile(join(options.outputDir, 'textures', sanitizeFilename(texture.name)), texture.data);
  ...
  const manifest = {
    ...
    source: { type: 'assetto-corsa-kn5', model: options.modelFilename, kn5Version: model.version },
    visualBinary: 'visual.bin', collisionBinary: 'collision.bin', collisionTriangleCount,
    meshes: meshManifest, materials: model.materials.map(packageMaterial), surfaces: packageSurfaces,
...
export function defaultTrackId(kn5Path: string): string {
  return basename(kn5Path, extname(kn5Path)).toLowerCase().replace(/[^a-z0-9]+/g, '-').replace(/^-|-$/g, '') || 'imported-track';
}

export function defaultOutputDirectory(kn5Path: string, id: string): string {
  return join(dirname(kn5Path), 'physics-drive-export', id);
}
